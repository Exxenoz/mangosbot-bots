...
namespace ai
{
	...
	{
	public:
		HealthInRangeTrigger(PlayerbotAI* ai, std::string name, float maxValue, float minValue = 0) :
    ...
    {
    public:
        PartyMemberLowHealthTrigger(PlayerbotAI* ai, std::string name = "party member low health", float value = sPlayerbotAIConfig.lowHealth, float minValue = sPlayerbotAIConfig.criticalHealth) :
            HealthInRangeTrigger(ai, name, value, minValue) {}
